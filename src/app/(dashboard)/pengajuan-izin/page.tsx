...
export default function PengajuanIzinPage() {
  ...
  const [form, setForm] = useState({
    ...
    tanggalMulai: "",
    tanggalSelesai: "",
    jenis: "Sakit (Dengan Surat Dokter)",
    alasan: "",
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (d.status === "success") {
        setSuccess("Pengajuan izin berhasil dikirim dan menunggu persetujuan!");
        setForm((f) => ({
          ...f,
          tanggalMulai: "",
          tanggalSelesai: "",
        ...
        setError(d.message ?? "Gagal mengajukan izin");
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Formulir Pengajuan Izin & Cuti</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          Pengajuan izin tidak hadir, sakit, atau cuti bagi Streamer dan Staff Operasional.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="mis. Demam tinggi dan disarankan dokter istirahat 2 hari."
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 sm:px-6 bg-slate-50/70 border-b border-slate-200 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 text-sm">Riwayat Pengajuan Izin ({history.length})</h3>
            <span className="text-xs text-slate-500">Status Pengajuan</span>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {history.map((h) => (
                  <tr key={h.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="px-4 py-3 text-slate-600">{h.jenis ?? "Izin"}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {new Date(h.tanggalMulai).toLocaleDateString("id-ID")} - {new Date(h.tanggalSelesai).toLocaleDateString("id-ID")}
                    </td>
